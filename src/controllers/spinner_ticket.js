...
export const getAllTickets = async (page, limit, filter) => {
  let response = await axios.get(
    `${BASE_URL}/spinner/ticket?page=${page}&limit=${limit}&filter=${filter}`
  );
...
export const getTurnOver = async (filter, user, from, to) => {
  let response = await axios.get(
    `${BASE_URL}/spinner/ticket/turnover?filter=${filter}&user=${user}&from=${from}&to=${to}`
  );
  if (response.status === 200 && !response.data.error) {
  }
...
export const addTicket = async (data) => {
  ...
  let response = await axios.post(`${BASE_URL}/spinner/ticket`, {
    game_id,
    money,
    value,
    lucky_number,
...
export const updateTicket = async (data) => {
  let response = await axios.patch(
    `${BASE_URL}/spinner/ticket/${data.id}`,
    data
  );
  return response.data;
};
