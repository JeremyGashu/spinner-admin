...
import { getAllKenoTickets } from "../controllers/keno_ticket";
...
const KenoTickets = () => {
  ...
  const { data, isLoading } = useQuery(["kenot", page, limit, filter], () =>
    getAllKenoTickets(page, limit, filter)
  );
  //   console.log("Loading tickets =>", data);
  useEffect(() => {
    queryClient.invalidateQueries(["kenot"]);
    return () => {};
  }, [page]);
  ...
  let columns = [
    {
      ...
      renderCell: (cellValue) => {
        return (
          <Typography sx={{ fontSize: 13, cursor: "pointer" }}>
            {padNumbers(cellValue["row"]["id"], 9)}
          </Typography>
        );
      },
    },

    {
      ...
      renderCell: (cellValue) => {
        ...
        // a.toLocaleString
        return (
          <Typography sx={{ fontSize: 13 }}>{`${new Date(
        ...
        return (
          <Typography
            sx={{
              ...
              px: 2,
              py: 0.5,
              backgroundColor: getBackgroudColorFromStatus(
        ...
        <>
            ...
            >
              ...
              {usersData &&
                usersData
                  ...
                  .map((user) => {
                    return (
                      <MenuItem key={user.username} value={user.name}>
                        {user.name}
                      </MenuItem>
                    );
                  })}
          ...
          <DataGrid
            ...
            rows={
              (data.data && user === ""
                ? data.data
                : data.data.filter((t) => t.cashier === user)) || []
            }
            columns={columns}
            // pagination={false}
